...
export function setDiscordClient(client) {
    ...
    try {
        ...
        if (forumMonitor) {
            forumMonitor.setWebhookCallback((issueNumber, threadId) => {
...
app.post('/api/sync/post', async (req, res) => {
    try {
        const { id, title, content, authorId, authorName, authorAvatar, channelId, guildId, tags = [], createdAt } = req.body;
...
app.post('/api/sync/message', async (req, res) => {
    try {
        const { id, content, authorId, authorName, authorAvatar, postId, guildId, createdAt } = req.body;
...
app.get('/api/stats', async (req, res) => {
    try {
        ...
        let prismaStats = {};
        try {
            const [totalPosts, totalMessages, totalChannels, recentSyncs] = await Promise.all([
