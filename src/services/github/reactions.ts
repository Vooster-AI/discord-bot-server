...
export class ReactionManager {
    ...
    public async handleReaction(
        ...
        try {
            ...
            if (added) {
                ...
                await this.removeReaction(targetUrl, targetType, githubReaction, commentId);
            }
    ...
    private async removeReaction(targetUrl: string, targetType: string, githubReaction: string, commentId?: number): Promise<void> {
        console.log(`🔍 [GITHUB DEBUG] GitHub 반응 제거 시도: ${githubReaction} from ${targetType}`);

        try {
            // targetUrl에서 reactions를 가져오기 위해 URL 수정
            const reactionsUrl = targetUrl; // 이미 /reactions가 포함된 URL
            ...
            if (userReaction) {
                ...
                const deleteUrl = commentId 
                    ? `${this.client.getBaseUrl()}/repos/${this.client.getRepository()}/issues/comments/${commentId}/reactions/${userReaction.id}`
                    : targetUrl.replace('/reactions', `/reactions/${userReaction.id}`);
                
                console.log(`🔍 [GITHUB DEBUG] 반응 제거 URL: ${deleteUrl}`);
                
                await axios.delete(deleteUrl, { headers: this.client.getReactionHeaders() });
                
                console.log(`✅ [GITHUB DEBUG] GitHub 반응 제거 완료: ${githubReaction} from ${targetType}`);
            } else {
                console.log(`ℹ️ [GITHUB DEBUG] 제거할 반응을 찾을 수 없습니다: ${githubReaction} by ${currentUser} in ${targetType}`);
                console.log(`🔍 [GITHUB DEBUG] 사용 가능한 반응들:`);
                reactionsResponse.data.forEach(reaction => {
                    console.log(`🔍 [GITHUB DEBUG] - ${reaction.content} by ${reaction.user.login} (ID: ${reaction.id})`);
                });
            }
